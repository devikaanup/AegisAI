            ...
            >
              <div className="flex items-center justify-between text-[11px] font-mono mb-1">
                <div className="flex items-center space-x-1.5 min-w-0">
                  ...
                  />
                  <span className={`font-bold truncate max-w-[120px] ${isSelected ? "text-cyan-300" : "text-slate-200"}`}>
                    {s.name}
                  </span>
                  {isSelected && (
                    <span className="px-1 py-0.2 rounded bg-cyan-500/20 text-cyan-300 text-[8px] font-bold uppercase tracking-wider shrink-0">
                      ASSIGNED
                    </span>
                  )}
                </div>
                <div className="flex items-center space-x-1.5 shrink-0">
                  <span className="tabular-nums font-mono text-[11px] text-slate-300 font-bold">
                    {s.currentOccupancy}/{s.capacity}
                  </span>
                  {isFull && (
                    <span className="px-1 py-0.2 rounded bg-rose-950 border border-rose-500 text-rose-300 text-[8px] font-extrabold tracking-wider">
                      FULL
                    </span>
                  )}
                </div>
