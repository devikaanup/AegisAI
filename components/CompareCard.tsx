...
export function CompareCard({
  ...
  const slackDisplay =
    route.minFloodSlackMin === 999 || route.minFloodSlackMin == null
      ? "Clear"
    ...
    return (
      <div className="absolute top-3 right-3 z-20 select-none">
        <button
          ...
          className="px-3 py-1.5 rounded-lg border border-emerald-500/50 bg-[#0c121d]/90 backdrop-blur-md shadow-xl text-xs font-mono text-emerald-300 hover:bg-[#141e2e] hover:border-emerald-400 transition-all flex items-center space-x-2.5 group cursor-pointer"
  ...
  return (
    <div className="absolute top-3 right-3 z-20 w-[300px] max-w-[calc(100%-1.5rem)] rounded-xl border border-[#25354c] bg-[#0c121d]/95 backdrop-blur-md p-3 shadow-2xl text-slate-100 select-none transition-all animate-in fade-in duration-150">
      ...
      <div className="flex items-center justify-between border-b border-[#1e293b] pb-2 mb-2.5">
        ...
        <div className="flex items-center space-x-1.5">
          ...
          <button
            ...
            className="px-2 py-0.5 rounded bg-[#141e2e] hover:bg-[#1a283d] border border-slate-700 text-slate-300 hover:text-white text-xs font-mono transition-colors cursor-pointer"
        ...
        <div className="p-2.5 rounded-lg bg-emerald-950/25 border border-emerald-500/35 space-y-1">
          ...
          <div className="pt-1.5 text-[10px] font-mono text-emerald-400 border-t border-emerald-900/40 flex items-start space-x-1">
            ...
            <span className="line-clamp-2 leading-tight">
              {route.safetyStatus ?? "SAFE"} · {slackDisplay}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 border-t border-[#1e293b] pt-2">
        ...
        <button
          onClick={() => setIsMinimized(true)}
          className="text-slate-400 hover:text-emerald-300 underline underline-offset-2 cursor-pointer"
        >
          Collapse to Map
        </button>
      </div>
